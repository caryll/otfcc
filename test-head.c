...
int main(int argc, char *argv[]) {
	...
	for (uint16_t j = 0; j < font->glyf->numberGlyphs; j++) {
		...
		printf("GLYPH %5d : width %5d, %5d contours, %5d references\n", j, g.advanceWidth, g.numberOfContours, g.numberOfReferences);
		if (g.numberOfContours > 0) {
			for (uint16_t k = 0; k < g.numberOfContours; k++) {
				...
				for(uint16_t m = 0; m < g.content.contours[k].pointsCount; m++){
					printf("        %s (%5d, %5d)\n", g.content.contours[k].points[m].onCurve ? "ON " : "OFF", g.content.contours[k].points[m].x, g.content.contours[k].points[m].y);
				}
			}
			...
			for (uint16_t k = 0; k < g.numberOfReferences; k++) {
				printf("    REF %5d %5d %5d %5g %5g %5g %5g\n", 
					...
					g.content.references[k].x,
					g.content.references[k].y,
					g.content.references[k].a,
					g.content.references[k].b,
					g.content.references[k].c,
					g.content.references[k].d);
			}
		}
	}
	*/
