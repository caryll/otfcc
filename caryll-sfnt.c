...
static void caryll_read_packets(caryll_sfnt *font, FILE *file) {
	for (uint32_t count = 0; count < font->count; count++) {
		...
		font->packets[count].pieces = (caryll_piece *)malloc(
		    sizeof(caryll_piece) * font->packets[count].numTables);
		...
		for (uint32_t i = 0; i < font->packets[count].numTables; i++) {
			...
			font->packets[count].pieces[i].data = (uint8_t *)malloc(
			    sizeof(uint8_t) * font->packets[count].pieces[i].length);
		}

		for (uint32_t i = 0; i < font->packets[0].numTables; i++) {
			(void)fseek(file, font->packets[count].pieces[i].offset, SEEK_SET);
			(void)fread(font->packets[count].pieces[i].data,
			            font->packets[count].pieces[i].length, 1, file);
		}
	}
}
...
caryll_sfnt *caryll_sfnt_open(const char *path) {
	...
	case 'typ1':
		...
		font->packets =
		    (caryll_packet *)malloc(sizeof(caryll_packet) * font->count);
		font->offsets[0] = 0;
