...
const char *fsTypeLabels[] = {
    "_reserved1", "restrictedLicense", "previewPrintLicense", "editableEmbedding",   "_reserved2", "_reserved3",
    "_reserved4", "_reserved5",        "noSubsetting",        "bitmapEmbeddingOnly", NULL};
const char *fsSelectionLabels[] = {"italic",  "underscore",     "negative", "outlined", "strikeout", "bold",
                                   "regular", "useTypoMetrics", "wws",      "oblique",  NULL};
const char *codePageLabels1[] = {"latin1",        "latin2",     "cyrillic", "greek",  "turkish", "hebrew", "arabic",
...
const char *codePageLabels2[] = {"oem8",  "oem9",  "oem10", "oem11", "oem12", "oem13", "oem14", "oem15", "oem16",
...
void caryll_OS_2_to_json(table_OS_2 *table, json_value *root, const caryll_options *options) {
	...
	json_object_push(os_2, "ulUnicodeRange1", caryll_flags_to_json(table->ulUnicodeRange1, unicodeRangeLabels1));
	json_object_push(os_2, "ulUnicodeRange2", caryll_flags_to_json(table->ulUnicodeRange2, unicodeRangeLabels2));
	json_object_push(os_2, "ulUnicodeRange3", caryll_flags_to_json(table->ulUnicodeRange3, unicodeRangeLabels3));
	json_object_push(os_2, "ulUnicodeRange4", caryll_flags_to_json(table->ulUnicodeRange4, unicodeRangeLabels4));
