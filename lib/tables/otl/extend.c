...
static otl_Subtable *_caryll_read_otl_extend(font_file_pointer data, uint32_t tableLength, uint32_t subtableOffset,
                                             otl_LookupType BASIS, const otfcc_Options *options) {
	...
	subtable->subtable = otfcc_readOtl_subtable(data, tableLength, subtableOffset + read_32u(data + subtableOffset + 4),
	                                            subtable->type, options);
	goto OK;
FAIL:
	FREE(_subtable);
OK:
