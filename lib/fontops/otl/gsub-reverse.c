...
bool consolidate_gsub_reverse(caryll_font *font, table_otl *table, otl_subtable *_subtable,
	...
	if (subtable->inputIndex >= subtable->matchCount) {
		subtable->inputIndex = subtable->matchCount - 1;
	}
