...
cff_blob * compile_index (CFF_INDEX index)
{
  ...
  if (index.count > 0)
  {
    ...
    if (index.data != NULL)
      memcpy(blob->data + 3 + ((index.count + 1) * index.offSize),
        index.data, index.offset[index.count] - 1);
  }
...
{
  int32_t private_len = parse_dict_key(f->top_dict.data,
    f->top_dict.offset[1] - f->top_dict.offset[0], op_Private, 0).i;
  int32_t private_off = parse_dict_key(f->top_dict.data,
      f->top_dict.offset[1] - f->top_dict.offset[0], op_Private, 1).i;
...
static cff_blob * compile_rrcurveto (double x1, double y1, double x2, double y2, double x3, double y3)
{
  cff_blob * blob = calloc(1, sizeof(cff_blob));
...
{
  return compile_rrcurveto (
    outline->x[i] - outline->x[i - 1],
    outline->y[i] - outline->y[i - 1],
    outline->x[i + 1] - outline->x[i],
    outline->y[i + 1] - outline->y[i],
    outline->x[i + 2] - outline->x[i + 1],
    outline->y[i + 2] - outline->y[i + 1]);
}

static cff_blob * compile_endchar (void)
{
  return encode_cs2_operator(op_endchar);
}
...
static cff_blob * compile_single_glyph (CFF_File * f, uint16_t i)
{
  ...
  parse_outline(f->char_strings.data + f->char_strings.offset[i] - 1,
    f->char_strings.offset[i + 1] - f->char_strings.offset[i],
    f->global_subr, *temp_subr, &temp_stack, temp_outline);
...
static cff_blob * __rrcurveto (CFF_Outline * outline, uint16_t i)
{
  ...
  */
  ...
  sprintf(buf, "C %g %g %g %g %g %g\n",
    outline->x[i], outline->y[i],
    outline->x[i + 1], outline->y[i + 1],
    outline->x[i + 2], outline->y[i + 2]);
...
cff_blob * compile_cff (CFF_File * f)
{
  ...
  if (f->font_dict.count != 0 && f->fdselect.t != CFF_FDSELECT_UNSPECED)
  {
    ...
    if (s->size != 0)
    {
      int32_t off = h->size + n->size + 11 + t->size + i->size + delta + 3 + e->size + c->size;
      cff_blob * val = compile_offset(off);
    ...
    if (p->size != 0)
    {
      int32_t off = h->size + n->size + 11 + t->size + i->size + delta + 3 + e->size + c->size + s->size;
  ...
  else
  {
    ...
    if (s->size != 0)
    {
      int32_t off = h->size + n->size + 11 + t->size + i->size + delta + 3 + c->size + e->size;
      cff_blob * val = compile_offset(off);
      cff_blob * op = encode_cff_operator(op_CharStrings);
      blob_merge(blob, val);
      blob_merge(blob, op);
    ...
    if (r->size != 0)
    {
      int32_t off = h->size + n->size + 11 + t->size + i->size + delta + 3 + c->size + e->size + s->size;
      cff_blob * val = compile_offset(off);
