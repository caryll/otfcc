...
static void parse_index (uint8_t * data, uint32_t pos, CFF_INDEX * in)
{
  ...
  if (in->count > 0)
  {
    ...
    memcpy(in->data, data + pos + 3 + (in->count + 1) * in->offSize,
      in->offset[in->count] - 1);
  }
  else
  {
    in->offset = NULL;
  ...
  else
  {
    ...
    case 1:
      charsets->t = CFF_CHARSET_FORMAT1;
      {
        ...
        for (i = 0; ; i++)
          if (gu2(cff->raw_data, offset + 1 + i * 3) + gu1(cff->raw_data, offset + 3 + i * 3) == cff->char_strings.count - 1)
            break;
    ...
    case 2:
      charsets->t = CFF_CHARSET_FORMAT2;
      {
        ...
        for (i = 0; ; i++)
          if (gu2(cff->raw_data, offset + 1 + i * 4) + gu2(cff->raw_data, offset + 3 + i * 4) == cff->char_strings.count - 1)
            break;
...
static void parse_cff_bytecode (CFF_File * cff)
{
  ...
  if (cff->top_dict.data != NULL)
  {
    ...
    //cff->topdict.dict = parse_dict(cff->top_dict.data, cff->top_dict.offset[1] - cff->top_dict.offset[0]);

    /* CharString INDEX */
    offset = parse_dict_key(cff->top_dict.data,
      cff->top_dict.offset[1] - cff->top_dict.offset[0],
      op_CharStrings, 0).i;
    ...
    /* Encodings */
    offset = parse_dict_key(cff->top_dict.data,
      cff->top_dict.offset[1] - cff->top_dict.offset[0], op_Encoding, 0).i;
    ...
    /* Charsets */
    offset = parse_dict_key(cff->top_dict.data,
      cff->top_dict.offset[1] - cff->top_dict.offset[0], op_charset, 0).i;
    ...
    /* FDSelect */
    offset = parse_dict_key(cff->top_dict.data,
      cff->top_dict.offset[1] - cff->top_dict.offset[0], op_FDSelect, 0).i;
    ...
    /* Font Dict INDEX */
    offset = parse_dict_key(cff->top_dict.data,
      cff->top_dict.offset[1] - cff->top_dict.offset[0], op_FDArray, 0).i;
  ...
  /* Private and Local Subr */
  {
    ...
    if (cff->top_dict.data != NULL)
      private_len = parse_dict_key(cff->top_dict.data,
        cff->top_dict.offset[1] - cff->top_dict.offset[0], op_Private, 0).i,
      private_off = parse_dict_key(cff->top_dict.data,
        cff->top_dict.offset[1] - cff->top_dict.offset[0], op_Private, 1).i;
  ...
  if (file != NULL)
  {
    ...
    switch (file->encodings.t)
    {
      ...
      case CFF_ENC_FORMAT0:
        if (file->encodings.f0.code != NULL)
          free(file->encodings.f0.code);
        break;
      case CFF_ENC_FORMAT1:
        if (file->encodings.f1.range1 != NULL)
          free(file->encodings.f1.range1);
        break;
      case CFF_ENC_FORMAT_SUPPLEMENT:
        if (file->encodings.ns.supplement != NULL)
          free(file->encodings.ns.supplement);
        break;
    }
    ...
    switch (file->charsets.t)
    {
      ...
      case CFF_CHARSET_FORMAT0:
        if (file->charsets.f0.glyph != NULL)
          free(file->charsets.f0.glyph);
        break;
      case CFF_CHARSET_FORMAT1:
        if (file->charsets.f1.range1 != NULL)
          free(file->charsets.f1.range1);
        break;
      case CFF_CHARSET_FORMAT2:
        if (file->charsets.f2.range2 != NULL)
          free(file->charsets.f2.range2);
        break;
    }
    ...
    switch (file->fdselect.t)
    {
      case CFF_FDSELECT_FORMAT0:
        if (file->fdselect.f0.fds != NULL)
          free(file->fdselect.f0.fds);
        break;
      case CFF_FDSELECT_FORMAT3:
        if (file->fdselect.f3.range3 != NULL)
          free(file->fdselect.f3.range3);
        break;
...
void parse_subr (uint16_t idx, uint8_t * raw, CFF_INDEX fdarray, CFF_FDSelect select, CFF_INDEX * subr)
{
  uint8_t fd;
  ...
  off_private = parse_dict_key(fdarray.data + fdarray.offset[fd] - 1,
    fdarray.offset[fd + 1] - fdarray.offset[fd], op_Private, 1).i;
  len_private = parse_dict_key(fdarray.data + fdarray.offset[fd] - 1,
    fdarray.offset[fd + 1] - fdarray.offset[fd], op_Private, 0).i;
          ...
          case op_cntrmask:
            {
              ...
              for (int i = 0; i < hint; i++)
                print_hintmask(*(start + advance + i));
...
static void outline_put_point (CFF_Outline * outline, double x, double y, uint8_t t)
{
  ...
  if (outline->cnt_point == 1)
    outline->x[outline->cnt_point - 1] = x,
    outline->y[outline->cnt_point - 1] = y;
  else
    outline->x[outline->cnt_point - 1] = outline->x[outline->cnt_point - 2] + x,
    outline->y[outline->cnt_point - 1] = outline->y[outline->cnt_point - 2] + y;

  outline->t[outline->cnt_point - 1] = t;
}

static void outline_put_bezier (CFF_Outline * outline, double x1, double y1, double x2, double y2, double x3, double y3)
{
...
void parse_outline (uint8_t * data, uint32_t len, CFF_INDEX gsubr, CFF_INDEX lsubr, CFF_Stack * stack, CFF_Outline * outline)
{
  uint16_t gsubr_bias = compute_subr_bias(gsubr.count);
  uint16_t lsubr_bias = compute_subr_bias(lsubr.count);
  uint8_t * start = data;
        ...
        switch (val.i)
        {
          ...
          case op_rmoveto:
            if (stack->index != 2)
              outline->width = stack->stack[0].d;
            outline_put_contour(outline, outline->cnt_point);
            outline_put_point(outline, stack->stack[stack->index - 2].d, stack->stack[stack->index - 1].d, 1);
            stack->index = 0;
            break;
          case op_hmoveto:
            if (stack->index != 1)
          ...
          case op_rrcurveto:
            for (i = 0; i < stack->index; i += 6)
              outline_put_bezier(outline, stack->stack[i].d, stack->stack[i + 1].d,
                stack->stack[i + 2].d, stack->stack[i + 3].d, stack->stack[i + 4].d, stack->stack[i + 5].d);
            stack->index = 0;
            break;
          case op_rcurveline:
          ...
          case op_rlinecurve:
            for (i = 0; i < stack->index - 6; i += 2)
              outline_put_point(outline, stack->stack[i].d, stack->stack[i + 1].d, 1);
            outline_put_bezier(outline,
              stack->stack[stack->index - 6].d, stack->stack[stack->index - 5].d,
            ...
            if (stack->index % 4 == 1)
            {
              ...
              for (i = 5; i < stack->index; i += 4)
                outline_put_bezier(outline, 0.0, stack->stack[i].d,
                  stack->stack[i + 1].d, stack->stack[i + 2].d, 0.0, stack->stack[i + 3].d);
            }
            else
            {
            ...
            if (stack->index % 4 == 1)
            {
              ...
              for (i = 5; i < stack->index; i += 4)
                outline_put_bezier(outline, stack->stack[i].d, 0.0,
                  stack->stack[i + 1].d, stack->stack[i + 2].d, stack->stack[i + 3].d, 0.0);
            }
            else
            {
              for (i = 0; i < stack->index; i += 4)
          ...
          case op_vhcurveto:
            ...
            if (stack->index % 8 == 5)
            {
              outline_put_bezier(outline, 0.0, stack->stack[stack->index - 5].d,
                stack->stack[stack->index - 4].d, stack->stack[stack->index - 3].d,
                stack->stack[stack->index - 2].d, stack->stack[stack->index - 1].d);
            }
            if (stack->index % 8 == 1)
            {
              outline_put_bezier(outline, stack->stack[stack->index - 5].d, 0.0,
                stack->stack[stack->index - 4].d, stack->stack[stack->index - 3].d,
          ...
          case op_hvcurveto:
            ...
            break;
          case op_hflex:
            outline_put_bezier(outline, stack->stack[0].d, 0.0,
              stack->stack[1].d, stack->stack[2].d, stack->stack[3].d, 0.0);
            outline_put_bezier(outline, stack->stack[4].d, 0.0,
              stack->stack[5].d, -stack->stack[2].d, stack->stack[6].d, 0.0);
            stack->index = 0;
            break;
          case op_flex:
            outline_put_bezier(outline, stack->stack[0].d, stack->stack[1].d,
              stack->stack[2].d, stack->stack[3].d, stack->stack[4].d, stack->stack[5].d);
            outline_put_bezier(outline, stack->stack[6].d, stack->stack[7].d,
              stack->stack[8].d, stack->stack[9].d, stack->stack[10].d, stack->stack[11].d);
            stack->index = 0;
            break;
          case op_hflex1:
            outline_put_bezier(outline, stack->stack[0].d, stack->stack[1].d,
              stack->stack[2].d, stack->stack[3].d, stack->stack[4].d, 0.0);
            outline_put_bezier(outline, stack->stack[5].d, 0.0,
              stack->stack[6].d, stack->stack[7].d, stack->stack[8].d, 
