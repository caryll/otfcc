...
otl_subtable *caryll_read_otl_extend(font_file_pointer data, uint32_t tableLength,
	...
	subtable->type =
	    read_16u(data + subtableOffset + 2) +
	    (lookupType == otl_type_gsub_extend
	         ? otl_type_gsub_unknown
	         : lookupType == otl_type_gpos_extend ? otl_type_gpos_unknown : otl_type_unknown);
	subtable->subtable = caryll_read_otl_subtable(
